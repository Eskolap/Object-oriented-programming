    ...
    class Store : IProducts, ICustomers
        {
            ...
            public void PrintProducts()
            {
                ...
                {
                    Console.WriteLine($"Tuote: {p.name}\tMäärä: {p.amount}\tHinta: {p.price.ToString("c", CultureInfo.CurrentCulture)}\tYhteensä: {p.CalculateTotal()}");
                }
            ...
            public override string ToString()
            {
                return $"Kaupan nimi: {name}\n" +
                    $"Tulos: {revenue.ToString("c", CultureInfo.CurrentCulture)}\n" +
